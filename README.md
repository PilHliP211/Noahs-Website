...
Submit shows through the private Google Form. The response Sheet mirrors submitted fields into the `Shows` tab and sets `Status` to `published` by default. To revoke a show, enter `draft`, `canceled`, or another non-`published` value in `Status override`. The generated `data/shows.json` file uses this shape:
...
Required show fields are `date`, `venue`, `city`, and `state`; `status` is generated by the Sheet. Shows older than yesterday are hidden automatically. If no future shows are listed, the site displays a coming-soon message.
...
```text
Show date | Venue | City | State or region | Doors time | Show time | Ticket URL | Event URL | Notes | Status | Status override
```

Column K is an optional `Status override` column. `Status` publishes by default and uses the override when one is provided. Only rows whose resolved `Status` is `published` are published.
