...
The show management flow uses a private Google Form and Google Sheet. GitHub Actions reads the `Form Responses 1` tab during deployment and writes the static `data/shows.json` file used by the website.
...
Submit shows through the private Google Form. Use the prefilled form link so `Status` starts as `published`; change it to `draft` when a submitted show should not appear on the site. The generated `data/shows.json` file uses this shape:
...
Required fields are `date`, `venue`, `city`, `state`, and `status`. Shows older than yesterday are hidden automatically. If no future shows are listed, the site displays a coming-soon message.
...
The workflow reads the Google Forms response tab named `Form Responses 1`. It should use these headers:

```text
Timestamp | Show date | Venue | City | State or region | Doors time | Show time | Ticket URL | Event URL | Notes | Status
```

Only rows with `Status` set to `published` are published. `draft` rows are ignored.
...
```text
GOOGLE_SHEET_RANGE='Form Responses 1'!A:K
